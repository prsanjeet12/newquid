...
import { TableHead, TableRow, TableCell, TableBody, Checkbox, Typography, IconButton, Button ,Box} from '@mui/material';
...
const MyTable: React.FC = () => {
  ...
  const buttonsData = [
    { src: '/Vector.png', alt: 'Filter', text: 'Filter' },
    { src: '/Frame (27).png', alt: 'Sort By', text: 'Sort By' },
  ];
  
  const statsData = [
    { label: 'UPDATED', value: '00' },
    { label: 'NEW', value: '00' },
    { label: 'TOTAL', value: '00' },
  ];
  ...
  return (
    ...
    <Box sx={{ display: 'flex', justifyContent: 'flex-start' }}>
      {buttonsData.map(({ src, alt, text }, index) => (
        <button key={index} style={{ display: 'flex', alignItems: 'center', backgroundColor: '#FFFFFF', border: '1px solid #D9DCDF', borderRadius: '8px', marginRight: '10px', padding: '8px' }}>
          <Image src={src} alt={alt} width={11} height={12} style={{ marginRight: '5px' }} />
          <Typography variant="body2" sx={{ fontSize: '13px', color: '#133039' }}>{text}</Typography>
        </button>
      ))}
    </Box>
    <Box sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }}>
      <button style={{ display: 'flex', alignItems: 'center', marginRight: '20px', backgroundColor: 'transparent', border: 'none' }}>
        <Image src="/Frame (28).png" width={24} height={24} alt="Icon" />
      </button>
      {statsData.map(({ label, value }, index) => (
        <Typography key={index} sx={{ fontWeight: 600, fontSize: '14px', marginRight: '20px' }}>
          <span>{value}:</span> <span style={{ color: '#95A2A6' }}>{label}</span>
        </Typography>
      ))}
    </Box>
  </Box>
